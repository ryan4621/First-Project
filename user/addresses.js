...
addAddressBtn.addEventListener('click', () => {
    ...
    document.querySelector('.address-modal-header h3').textContent = 'Add New Address';
...
function displayAddresses(addresses) {
    ...
    const addressCards = addresses.map(address => `
        <div class="card" data-address-id="${address.id}">
            <p><strong>${address.full_name}</strong> ${address.is_default ? '<span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: 0.8rem;">DEFAULT</span>' : ''}</p>
            <p>${address.street}</p>
            <p>${address.city}${address.state ? ', ' + address.state : ''} ${address.postal_code}</p>
            <p>${address.country}</p>
            ${address.phone ? `<p>Phone: ${address.phone}</p>` : ''}
            <div class="card-actions">
                <button class=" btn-secondary edit-address-btn" data-address-id="${address.id}">Edit</button>
                <button class="btn btn-small btn-danger delete-address-btn" data-address-id="${address.id}">Delete</button>
                ${!address.is_default ? `<button class="btn btn-small set-default-btn" data-address-id="${address.id}">Set Default</button>` : ''}
            </div>
        </div>
    `).join('');
...
addressForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        showToast(currentMode === 'edit' ? 'Failed to update address. Please try again.' : 'Failed to save address. Please try again.', 'error');
    } finally {
        saveBtn.disabled = false;
        saveBtn.textContent = currentMode === 'edit' ? 'Update Address' : 'Save Address';
    }
});
...
async function populateEditForm(addressData) {
    try {
        ...
        document.querySelector('.address-modal-header h3').textContent = 'Edit Address';
