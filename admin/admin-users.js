...
(() => {
  ...
  async function loadUsers() {
    ...
    try {
      const res = await fetch(`${apiBase}/users?${params.toString()}`, { credentials: "include" });
  ...
  function renderTable(users) {
    ...
    users.forEach(user => {
      ...
      tr.innerHTML = `
        ...
        <td class="actions">
          <a class="action-btn edit" href="edit-user.html?id=${user.id}">Edit</a>
          <button class="action-btn view" onclick="location.href='user-overview.html?id=${user.id}'">Overview</button>
  ...
  function renderPagination() {
    ...
    prev.disabled = cp <= 1;
    prev.onclick = () => { state.page = Math.max(1, cp - 1); loadUsers(); };
    ...
    for (let i = start; i <= end; i++) {
      ...
      b.textContent = i;
      b.onclick = () => { state.page = i; loadUsers(); };
    ...
    next.disabled = cp >= tp;
    next.onclick = () => { state.page = Math.min(tp, cp + 1); loadUsers(); };
  ...
  exportPdfBtn.addEventListener("click", async () => {
    try {
      ...
      const params = new URLSearchParams({ q: state.q, role: state.role, sort: state.sort });
      const res = await fetch(`${apiBase}/users/export?${params.toString()}`, { credentials: "include" });
  ...
  (function init() {
    ...
    limitSelect.addEventListener("change", () => { state.limit = parseInt(limitSelect.value, 10); state.page = 1; loadUsers(); });
    sortSelect.addEventListener("change", () => { state.sort = sortSelect.value; state.page = 1; loadUsers(); });
    roleFilter.addEventListener("change", () => { state.role = roleFilter.value; state.page = 1; loadUsers(); });
    loadUsers();
  })();
