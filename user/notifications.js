...
class UserNotifications {
	...
	bindEventListeners() {
		...
		document
			.getElementById("notifications-status-filter")
		...
		document
			.getElementById("notifications-category-filter")
			.addEventListener("change", (e) => {
		...
		// Action buttons
		document
			.getElementById("notifications-refresh-btn")
			.addEventListener("click", () => {
				this.refreshNotifications();
			});

		document
			.getElementById("notifications-mark-all-read-btn")
			.addEventListener("click", () => {
				this.markAllAsRead();
			});
		...
		// Bulk actions
		document
			.getElementById("notifications-select-all")
			.addEventListener("change", (e) => {
				this.toggleSelectAll(e.target.checked);
			});

		document
			.getElementById("notifications-bulk-mark-read")
			.addEventListener("click", () => {
				this.bulkMarkAsRead();
			});

		document
			.getElementById("notifications-bulk-delete")
			.addEventListener("click", () => {
				this.bulkDelete();
			});
		...
		// Pagination
		document
			.getElementById("notifications-prev-btn")
			.addEventListener("click", () => {
		...
		document
			.getElementById("notifications-next-btn")
			.addEventListener("click", () => {
				this.currentPage++;
				this.loadNotifications();
			});
	...
	showLoadingState() {
		document.getElementById("notifications-loading-state").style.display =
			"block";
	...
	showErrorState(message) {
		document.getElementById("notifications-loading-state").style.display =
			"none";
		document.getElementById("notifications-empty-state").style.display =
			"block";
		document.getElementById("notifications-container").style.display = "none";
		document.getElementById("notifications-pagination").style.display = "none";
	...
	renderNotifications() {
		...
		if (this.notifications.length === 0) {
			...
			document.getElementById("notifications-empty-state").style.display =
				"block";
			document.getElementById("notifications-container").style.display = "none";
			document.getElementById("notifications-pagination").style.display =
				"none";
			return;
		}
		...
		if (this.notifications.length > 0) {
			document.getElementById("notifications-bulk-actions").style.display =
				"flex";
		}

		// Update displayed count
		document.getElementById(
			"notifications-displayed-count"
		).textContent = `${this.notifications.length} notifications`;
	...
	renderNotificationItem(notification) {
		...
		return `
        <div class="notifications-item ${isUnread ? "unread" : ""}" data-id="${
			notification.id
		}">
          <div class="notifications-item-content">
            <div class="notifications-item-header">
              ...
              <div class="notifications-item-meta">
                <span class="notifications-category-badge notifications-category-${notification.category.replace(
									"_",
									"-"
								)}">
                  ${category}
            ...
            <div class="notifications-item-actions">
              <label class="notifications-bulk-checkbox" style="margin-right: 1rem;">
                <input type="checkbox" onchange="userNotifications.toggleNotificationSelection(${
									notification.id
								}, this.checked)">
                <span>Select</span>
              </label>
              ...
              <button class="notifications-action-btn notifications-action-delete" 
                      onclick="userNotifications.deleteNotification(${
												notification.id
											})">
                Delete
              </button>
	...
	updatePagination() {
		...
		document.getElementById("notifications-prev-btn").disabled =
			this.currentPage === 1;
	...
	generatePageNumbers(totalPages) {
		...
		for (let page = startPage; page <= endPage; page++) {
			...
			pageBtn.className = `notifications-pagination-btn ${
	...
	async markAsRead(notificationId) {
		try {
			...
			if (notificationElement) {
				...
				const markReadBtn = notificationElement.querySelector(
	...
	toggleNotificationSelection(notificationId, isSelected) {
		...
		selectAllCheckbox.checked =
			selectedCount === totalNotifications && totalNotifications > 0;
		selectAllCheckbox.indeterminate =
			selectedCount > 0 && selectedCount < totalNotifications;
	}
	...
	toggleSelectAll(selectAll) {
		...
		const checkboxes = document.querySelectorAll(
			'.notifications-item input[type="checkbox"]'
		);
		checkboxes.forEach((checkbox) => {
			checkbox.checked = selectAll;
			if (selectAll) {
				const notificationId = parseInt(
					checkbox.closest(".notifications-item").dataset.id
				);
				this.selectedNotifications.add(notificationId);
			}
		});
	...
	async bulkMarkAsRead() {
		...
		try {
			...
			this.loadNotifications();
			this.showToast(
				`${this.selectedNotifications.size} notifications marked as read`,
				"success"
			);
		} catch (error) {
	...
	async bulkDelete() {
		...
		try {
			...
			this.loadNotifications();
			this.showToast(
				`${this.selectedNotifications.size} notifications deleted`,
				"success"
			);
		} catch (error) {
