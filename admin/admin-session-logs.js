...
function setupEventListeners() {
	...
	document
		.getElementById("admin-session-logs-apply-filters")
	...
	document
		.getElementById("admin-session-logs-reset-filters")
		.addEventListener("click", () => {
			resetFilters();
		});
	...
	// Export button
	document
		.getElementById("admin-session-logs-export-btn")
		.addEventListener("click", () => {
			exportLogs();
		});
	...
	// Pagination buttons
	document
		.getElementById("admin-session-logs-prev-btn")
		.addEventListener("click", () => {
			if (currentPage > 1) {
				currentPage--;
				loadActivityLogs();
	...
	document
		.getElementById("admin-session-logs-next-btn")
		.addEventListener("click", () => {
			currentPage++;
			loadActivityLogs();
		});
	...
	// Enter key on search
	document
		.getElementById("admin-session-logs-search")
...
async function loadAdminList() {
	try {
		...
		const adminSelect = document.getElementById(
			"admin-session-logs-admin-filter"
		);
...
function applyFilters() {
	currentFilters = {
		...
		entity_type: document.getElementById("admin-session-logs-entity-filter")
			.value,
	...
	try {
      ...
      <tr>
        ...
        <td>
          ...
          <small style="color: #6b7280;">${escapeHtml(
						log.admin_email || ""
					)}</small>
        </td>
        ...
        <td>
          ${
						log.entity_type
							? `<span class="admin-session-logs-badge admin-session-logs-badge-${log.entity_type.replace(
									"_",
									""
							  )}">${formatEntityType(log.entity_type)}</span>`
							: "-"
					}
        </td>
        <td>
        ...
        <td>
          <button class="admin-session-logs-view-btn" onclick="viewLogDetails(${
						log.id
					})">
            View Details
          </button>
...
function updatePagination(pagination) {
	...
	document.getElementById(
		"admin-session-logs-page-info"
	).textContent = `Showing ${start} - ${end} of ${total} logs`;
...
async function viewLogDetails(logId) {
	...
	try {
      ...
      <div class="admin-session-logs-detail-row">
        ...
        <div class="admin-session-logs-detail-value">${formatAction(
					log.action
				)}</div>
      </div>
      ...
      <div class="admin-session-logs-detail-row">
        ...
        <div class="admin-session-logs-detail-value">${
					log.entity_type ? formatEntityType(log.entity_type) : "-"
				}</div>
      </div>
      ...
      <div class="admin-session-logs-detail-row">
        ...
        <div class="admin-session-logs-detail-value">${escapeHtml(
					log.request_path || "-"
				)}</div>
      </div>
      ...
      <div class="admin-session-logs-detail-row">
        ...
        <div class="admin-session-logs-detail-value">${formatTimestamp(
					log.created_at
				)}</div>
      </div>
      ...
      ${
        ...
        <div class="admin-session-logs-detail-row">
          ...
          <div class="admin-session-logs-detail-code">${escapeHtml(
    ...
    `;
		...
		modalBody.innerHTML =
			'<p style="text-align: center; color: #ef4444; padding: 20px;">Failed to load log details</p>';
	}
}
