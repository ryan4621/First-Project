...
if (document.readyState == 'loading') {
  ...
  // Create individual product element - FIXED: Use product_id instead of id
  ...
  async function purchaseClicked() {
    ...
    try {
      ...
      window.location.href = 'https://localhost:3000/checkout.html';
      return;
    } catch (error) {
  ...
  async function removeCartItem(event) {
    ...
    if (item.length === 0) {
      ...
      if (wlheader) wlheader.style.display = 'block';
    }
    ...
    if (item.length === 0){
      ...
      if (bagFooter) bagFooter.style.display = 'none';
    }
