...
function setupEventListeners() {
	document
		.getElementById("error-logs-apply-filters")
	...
	document
		.getElementById("error-logs-reset-filters")
		.addEventListener("click", resetFilters);
	document
		.getElementById("error-logs-export-btn")
		.addEventListener("click", exportLogs);

	document
		.getElementById("error-logs-prev-btn")
		.addEventListener("click", () => {
			if (currentPage > 1) {
				currentPage--;
				loadErrorLogs();
	...
	document
		.getElementById("error-logs-next-btn")
		.addEventListener("click", () => {
			currentPage++;
			loadErrorLogs();
		});
	...
	document
		.getElementById("error-logs-search")
	...
	detailModal
		.querySelector(".error-logs-modal-close")
		.addEventListener("click", () => {
	...
	resolveModal
		.querySelector(".error-logs-resolve-close")
		.addEventListener("click", () => {
			resolveModal.style.display = "none";
		});
	document
		.getElementById("error-logs-cancel-resolve")
		.addEventListener("click", () => {
			resolveModal.style.display = "none";
		});
	document
		.getElementById("error-logs-confirm-resolve")
		.addEventListener("click", confirmErrorResolve);
...
async function loadStatistics() {
	try {
		...
		document.getElementById("error-logs-critical-count").textContent =
			stats.critical_errors.toLocaleString();
		document.getElementById("error-logs-high-count").textContent =
			stats.high_errors.toLocaleString();
		document.getElementById("error-logs-unresolved-count").textContent =
			stats.unresolved_errors.toLocaleString();
		document.getElementById("error-logs-today-count").textContent =
...
async function loadErrorLogs() {
	...
	try {
      ...
      <tr>
        ...
        <td>
          <span class="error-logs-severity-badge error-logs-severity-${
						error.severity
					}">
            ${error.severity}
          </span>
        ...
        <td>
          <div class="error-logs-message" title="${escapeHtml(
						error.error_message
					)}">
        ...
        <td>
          <span class="error-logs-status-badge error-logs-status-${
        ...
        <td>
          <div class="error-logs-action-btns">
            <button class="error-logs-btn error-logs-btn-primary" onclick="viewErrorDetails(${
							error.id
						})">
              View
            </button>
...
function updatePagination(pagination) {
	...
	).textContent = `Showing ${start} - ${end} of ${total} errors`;
...
async function viewErrorDetails(errorId) {
	...
	modalBody.innerHTML =
		'<p style="text-align: center; padding: 20px;">Loading details...</p>';
	...
	try {
      ...
      <div class="error-logs-detail-row">
        ...
        <div class="error-logs-detail-value"><strong>${escapeHtml(
					error.error_type
				)}</strong></div>
      </div>
      ...
      <div class="error-logs-detail-row">
        ...
        <div class="error-logs-detail-value">${escapeHtml(
					error.error_message
				)}</div>
      </div>
      ...
      ${
        ...
        <div class="error-logs-detail-row">
          ...
          <div class="error-logs-detail-code">${escapeHtml(
						error.stack_trace
					)}</div>
        </div>
      `
      ...
      ${
        ...
        <div class="error-logs-detail-row">
          ...
          <div class="error-logs-detail-code">${escapeHtml(
						formatJSON(error.request_query)
					)}</div>
        </div>
      `
					: ""
      ...
      ${
        ...
        <div class="error-logs-detail-row">
          ...
          <div class="error-logs-detail-code">${escapeHtml(
						formatJSON(error.additional_data)
					)}</div>
        </div>
      `
					: ""
      ...
      ${
        ...
        <div class="error-logs-detail-row">
          ...
          <div class="error-logs-detail-value">${formatTimestamp(
						error.resolved_at
					)}</div>
        </div>
        <div class="error-logs-detail-row">
          ...
          <div class="error-logs-detail-value">${escapeHtml(
						error.resolved_by_name || "Unknown"
					)}</div>
        </div>
        ${
    ...
    `;
		...
		modalBody.innerHTML =
			'<p style="text-align: center; color: #ef4444; padding: 20px;">Failed to load error details</p>';
	}
}
...
async function confirmErrorResolve() {
	const notes = document
		.getElementById("error-logs-resolution-notes")
		.value.trim();

	try {
		const response = await fetch(
			`${API_BASE}/error-logs/${currentErrorId}/resolve`,
			{
				method: "PUT",
				credentials: "include",
				headers: {
...
async function unresolveError(errorId) {
	...
	try {
		const response = await fetch(
			`${API_BASE}/error-logs/${errorId}/unresolve`,
			{
				method: "PUT",
				credentials: "include",
				headers: {
