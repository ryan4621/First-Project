...
document.addEventListener('DOMContentLoaded', function() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.user) {
            ...
            // Make fields readonly if auto-filled
            if (data.user.name) {
    ...
    function validateName() {
      ...
      if (!nameRegex.test(name)) {
        showError(nameError, 'Name can only contain letters, spaces, hyphens, apostrophes, and periods');
        return false;
      }
    ...
    function validateSubject() {
      ...
      const validSubjects = ['general', 'account', 'product', 'technical', 'billing', 'feedback', 'other'];
    ...
    function updateCharacterCount() {
      ...
      const existingCount = messageField.parentElement.querySelector('.contact-character-count');
      if (existingCount) {
    ...
    function clearError(errorElement) {
      ...
      errorElement.parentElement.querySelector('input, select, textarea').style.borderColor = '#e9ecef';
    }
    ...
    function showSuccessMessage(customMessage = null) {
      const message = customMessage || 'Message sent successfully! We\'ve received your inquiry and will respond within 24 hours.';
      ...
      try {
        ...
        if (response.ok && data.success) {
          ...
          if (data.data && data.data.estimatedResponse) {
            successMsg += ` Expected response time: ${data.data.estimatedResponse}.`;
          }
          ...
          if (data.errors && Array.isArray(data.errors)) {
            ...
            showGeneralError(data.message || 'Failed to send your message. Please try again.');
          }
        }
