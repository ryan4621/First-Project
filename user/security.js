...
export function initializeSecurity() {
	...
	function initializePasswordStrengthIndicator() {
		...
		newPasswordField.addEventListener("input", (e) => {
			...
			if (password.length > 0) {
				...
				const errorMsg = e.target.parentNode.querySelector(
					".security-error-message"
				);
				if (errorMsg) {
					errorMsg.remove();
		...
		enable2FABtn.addEventListener("click", async () => {
			try {
				const currentlyEnabled =
					enable2FABtn.textContent.trim() === "Disable 2FA";
				...
				const result = await show2FAConfirmationModal(
					newState,
					user.email,
					user.email_verified
				);
	...
	function show2FAConfirmationModal(enabling, userEmail, emailVerified) {
		return new Promise((resolve) => {
			...
			// Set modal content
			modalTitle.textContent = enabling
        ...
        `;
				...
				modalWarning.textContent =
					"Two-factor authentication will be disabled. You will no longer need a verification code to log in.";
			}

			passwordGroup.style.display = "block";
			confirmBtn.textContent = enabling ? "Enable 2FA" : "Disable 2FA";
			confirmBtn.className = enabling
				? "settings-btn settings-btn-primary"
				: "settings-btn settings-btn-warning";
	...
	function update2FAStatus(enabled) {
		...
		if (enabled) {
			...
			statusIndicator.className =
				"security-status-dot security-status-disabled";
	...
	async function initializeSecurityQuestions() {
		const setupQuestionsBtn = document.getElementById(
			"security-setup-questions-btn"
		);
		const questionsStatus = document.getElementById(
	...
	async function loadSecurityQuestionsStatus() {
		try {
			const response = await fetch(`/api/security-questions/status`, {
				method: "GET",
				credentials: "include",
			});
	...
	function updateSecurityQuestionsStatus(hasQuestions) {
		const statusIndicator = document.querySelector(
			"#security-questions-status .security-status-dot"
		);
		const statusText = document.querySelector(
			"#security-questions-status .security-status-text"
		);
	...
	async function saveSecurityQuestions() {
		...
		try {
			...
			if (
				...
				data.question1 === data.question3 ||
				data.question2 === data.question3
			) {
				showSecurityToast(
					"Please select different questions for each field",
					"error"
				);
				return;
			}
			...
			showSecurityToast(
				"Failed to save security questions: " + error.message,
				"error"
			);
		} finally {
	...
	function displaySessions(sessions) {
      ...
      <div class="security-session-item">
          ...
          <div class="security-session-details">
              <span class="security-session-location">${
								session.ip_address || "Unknown Location"
							}</span>
              <span class="security-session-time">${formatSessionTime(
								session.last_active
							)}</span>
          </div>
          </div>
	...
	function displayActivityLogs(activities, isInitial = false) {
      ...
      <div class="security-activity-item">
        <div class="security-activity-icon">${getActivityIcon(
        ...
        <div class="security-activity-info">
          ...
          <div class="security-activity-details">
            <span class="security-activity-time">${formatActivityTime(
							activity.created_at
						)}</span>
            ${
							activity.device_info
								? `<span class="security-activity-device">${activity.device_info}</span>`
								: ""
						}
            ${
							activity.ip_address
								? `<span class="security-activity-ip">${activity.ip_address}</span>`
								: ""
						}
          </div>
        </div>
	...
	function showAccountActionConfirmation(
		title,
		message,
		deactivateClicked,
		deleteClicked
	...
	async function deleteAccount(password) {
		try {
			...
			if (response.ok) {
				showSecurityToast(
					"Account deleted successfully. You have 30 days to recover it.",
					"success"
				);
				setTimeout(() => {
					window.location.href = "../frontend/farfetch.html";
