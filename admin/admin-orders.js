...
class AdminOrdersManager {
	...
	setupEventListeners() {
		...
		document
			.getElementById("status-close-modal")
			.addEventListener("click", () => {
		...
		document
			.getElementById("delete-close-modal")
			.addEventListener("click", () => {
                ...
                <div style="display: flex; justify-content: space-between; align-items: start; flex-wrap: wrap; gap: 15px;">
                    <div style="flex: 1; min-width: 250px;">
                        ...
                        <p style="margin: 5px 0;">
                            ...
                            <span class="admin-status-badge admin-status-${
															order.status
                    ...
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        <button class="admin-btn admin-btn-primary" onclick="adminOrdersManager.viewOrderDetails('${
													order.order_number
												}')">
                            View Details
                        </button>
                        <button class="admin-btn admin-btn-success" onclick="adminOrdersManager.openStatusModal('${
													order.order_number
												}')">
                            Change Status
                        </button>
                        <button class="admin-btn admin-btn-danger" onclick="adminOrdersManager.openDeleteModal('${
													order.order_number
												}')">
                            Delete Order
                        </button>
	...
	async viewOrderDetails(orderNumber) {
		try {
			const response = await fetch(`/admin/orders/${orderNumber}`, {
				credentials: "include",
			});
	...
	displayOrderDetails(data) {
		...
		modalBody.innerHTML = `
            <div style="margin-bottom: 20px;">
                ...
                <p><strong>Status:</strong> <span class="admin-status-badge admin-status-${
									order.status
								}">${this.formatStatus(order.status)}</span></p>
                <p><strong>Payment Status:</strong> <span class="admin-status-badge admin-status-${
									order.payment_status
            ...
            ${
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    ${payments
                        ...
                        ${
													payment.receipt_url
														? `<p><a href="${payment.receipt_url}" target="_blank">View Receipt</a></p>`
														: ""
												}
                    `
											)
                    ...
                    ${refunds
                        ...
                        ${
													refund.stripe_refund_id
														? `<p><strong>Stripe Refund ID:</strong> ${refund.stripe_refund_id}</p>`
														: ""
												}
                    `
											)
	...
	async updateOrderStatus() {
		...
		try {
			const response = await fetch(
				`/admin/orders/${this.currentOrderNumber}/status`,
				{
					method: "PUT",
					headers: {
	...
	async deleteOrder() {
		try {
			const response = await fetch(`/admin/orders/${this.currentOrderNumber}`, {
				method: "DELETE",
				credentials: "include",
				headers: {
					"x-csrf-token": window.getCsrfToken(),
