...
class NotificationManager {
	...
	bindEventListeners() {
		// Main action buttons
		document
			.getElementById("notification-create-btn")
			.addEventListener("click", () => {
				this.showCreateForm();
			});

		document
			.getElementById("notification-refresh-btn")
			.addEventListener("click", () => {
				this.refreshData();
			});

		document
			.getElementById("notification-templates-btn")
			.addEventListener("click", () => {
		...
		document
			.getElementById("notification-target-all")
			.addEventListener("change", (e) => {
				const userRolesSelect = document.getElementById(
		...
		// Modal controls
		document
			.getElementById("notification-modal-close")
			.addEventListener("click", () => {
				this.closeModal();
			});
	...
	updateStatsDisplay() {
		...
		document.getElementById("sent-notifications-count").textContent =
			overall.sent_notifications || 0;
		document.getElementById("draft-notifications-count").textContent =
			overall.draft_notifications || 0;
		document.getElementById("total-recipients-count").textContent =
			overall.total_recipients_reached || 0;

		// Calculate read rate percentage
		const readRate =
			engagement && engagement.total_user_notifications > 0
				? Math.round(
						(engagement.read_notifications /
							engagement.total_user_notifications) *
							100
				  )
				: 0;
	...
	showLoadingState() {
		document.getElementById("notification-loading-state").style.display =
			"block";
	...
	renderNotifications() {
		...
		if (this.notifications.length === 0) {
			document.getElementById("notification-loading-state").style.display =
				"none";
			document.getElementById("notification-empty-state").style.display =
				"block";
			document.getElementById("notification-table").style.display = "none";
			return;
		}
		...
		tbody.innerHTML = this.notifications
			.map((notification) => this.renderNotificationRow(notification))
			.join("");
	...
	renderNotificationRow(notification) {
      ...
      <tr>
        ...
        <td>
          <span class="notification-category-badge notification-category-${notification.category.replace(
						"_",
						"-"
					)}">
        ...
        <td>
          <span class="notification-status-badge notification-status-${
						notification.status
					}">
            ${notification.status}
          </span>
        ...
        <td>
          <div class="notification-actions-cell">
            <button class="notification-action-btn notification-action-view" 
                    onclick="notificationManager.viewNotification(${
											notification.id
										})" 
                    title="View Details">
              👁️
            ...
            <button class="notification-action-btn notification-action-delete" 
                    onclick="notificationManager.deleteNotification(${
											notification.id
										})" 
                    title="Delete">
              🗑️
	...
	collectFormData() {
		const targetAll = document.getElementById(
	...
	renderNotificationDetails(notification) {
		...
		).textContent = `Notification #${notification.id} - Details`;
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
          ...
          <div>
            ...
            <p><strong>Email Sent:</strong> ${
							notification.email_sent_count || 0
						}</p>
            <p><strong>Push Sent:</strong> ${
							notification.push_sent_count || 0
						}</p>
          </div>
        </div>
