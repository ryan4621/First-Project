...
document.addEventListener("DOMContentLoaded", async () => {
	function bannerEmailVerify() {
		document
			.getElementById("banner-verify-btn")
			.addEventListener("click", async () => {
				...
				try {
					...
					if (response.ok) {
						...
						showToast(
							data.message || "Please wait before requesting another email",
							"error"
						);
						btn.textContent = "Resend verification";
    ...
    <ul>
      <li id="display-name"><strong>Name:</strong> ${
				user.user.name || "Not set"
			}</li>
      <li id="display-email"><strong>Email:</strong> ${
				user.user.email || "Not set"
			}</li>
      <li id="display-phone"><strong>Phone:</strong> ${
				user.user.phone || "Not set"
			}</li>
      <li id="display-gender"><strong>Gender:</strong> ${
				user.user.gender || "Not set"
			}</li>
      <li id="display-country"><strong>Country:</strong> ${
	...
	const profileUploadInstructions = document.getElementById(
		"profileUploadInstructions"
	);
	...
	const profileCancelUploadBtn = document.getElementById(
		"profileCancelUploadBtn"
	);
	const profileReuploadBtn = document.getElementById("profileReuploadBtn");
	const profileProgressContainer = document.getElementById(
	...
	async function uploadProfileFile() {
		...
		try {
			...
			formData.append(
				"fileName",
				`profile-${Date.now()}-${profileSelectedFile.name}`
			);
	...
	class PaymentMethodManager {
		constructor() {
			...
			this.stripePublishableKey =
				"pk_test_51SAMlKAJvXS66ZSZFjuWB6eFvikg9ZO09VFJTCmSixa7JtqW7ya1zNilnBMUr3RxFjbBD6W8KyVP1zHFqWz9PzZT00SqysCRXu"; // Replace with your actual key
		...
		setupEventListeners() {
			...
			document
				.getElementById("add-payment-method-btn")
			...
			document
				.getElementById("payment-close-modal")
				.addEventListener("click", () => {
					this.closeModal();
				});

			document
				.getElementById("payment-cancel-btn")
				.addEventListener("click", () => {
					this.closeModal();
				});
			...
			document
				.getElementById("payment-add-form")
			...
			document
				.getElementById("payment-confirm-cancel")
				.addEventListener("click", () => {
			...
			try {
				...
				if (listElement) {
					listElement.innerHTML =
						'<p style="color: #dc3545;">Failed to load payment methods. Please refresh the page.</p>';
				}
			}
		}
        ...
        <div class="payment-method-card ${pm.is_default ? "default" : ""}">
          ...
          <div class="payment-method-actions">
            ${
							!pm.is_default
								? `<button class="btn-small btn-primary" onclick="paymentMethodManager.setDefault(${pm.id})">Set Default</button>`
								: ""
						}
            <button class="btn-small btn-danger" onclick="paymentMethodManager.deletePaymentMethod(${
							pm.id
						}, '${pm.card_brand} **** ${pm.card_last4}')">Delete</button>
          </div>
        </div>
      `
				)
		...
		async openAddPaymentMethodModal() {
			try {
				...
				this.showError(
					"Failed to initialize payment method form: " + error.message
				);
			}
		}
