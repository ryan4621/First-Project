...
class CheckoutManager {
	...
	renderAddressSelection(addresses) {
		...
		addresses.forEach((address) => {
			...
			option.textContent = `${address.full_name} - ${address.street}, ${
				address.city
			}, ${address.country}${address.is_default ? " (Default)" : ""}`;
	...
	renderPaymentMethodSelection() {
		const paymentSelection = document.getElementById(
			"payment-method-selection"
		);
		const noPaymentMessage = document.getElementById(
		...
		this.savedPaymentMethods.forEach((pm) => {
			...
			} •••• ${pm.card_last4} - Exp ${pm.card_exp_month}/${pm.card_exp_year}${
	...
	handlePaymentMethodSelection(paymentMethodId) {
		...
		const selectedPayment = this.savedPaymentMethods.find(
            ...
            <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <div>
                    <div style="font-weight: 600;">${
											selectedPayment.card_brand.charAt(0).toUpperCase() +
											selectedPayment.card_brand.slice(1)
										} •••• ${selectedPayment.card_last4}</div>
                    <div style="color: #666; font-size: 14px;">Expires ${
											selectedPayment.card_exp_month
										}/${selectedPayment.card_exp_year}</div>
                </div>
            </div>
        `;
	...
	showNoPaymentMethods() {
		...
		document.getElementById("no-payment-methods-message").style.display =
			"block";
	...
	renderOrderSummary() {
		...
		this.cart.items.forEach((item) => {
			...
			itemElement.innerHTML = `
                <img src="${
									item.image_url ||
									"data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iODAiIGhlaWdodD0iMTAwIiB2aWV3Qm94PSIwIDAgODAgMTAwIiBmaWxsPSJub25lIiB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPjxyZWN0IHdpZHRoPSI4MCIgaGVpZ2h0PSIxMDAiIGZpbGw9IiNmMGYwZjAiLz48L3N2Zz4="
								}" alt="${item.name}">
		...
		try {
			...
			if (paymentIntent.paymentIntent.status === "succeeded") {
				...
				const { error: confirmError, paymentIntent: confirmedPayment } =
					await this.stripe.confirmCardPayment(paymentData.clientSecret);
				...
				throw new Error(
					"Unexpected payment status: " + paymentIntent.paymentIntent.status
				);
			}
		} catch (error) {
