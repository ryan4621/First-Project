...
let currentUser = null;
    ...
    function populateUserData(data) {
      const { user, preferences, sessions, supportTickets, notifications, security, activity, addresses, stats } = data;
      ...
      const emailVerificationBadge = document.getElementById('email-verification-badge');
      if (user.email_verified) {
        emailVerificationBadge.innerHTML = '<span class="verification-badge verified">✓ Verified</span>';
      } else {
        emailVerificationBadge.innerHTML = '<span class="verification-badge not-verified">⚠ Not Verified</span>';
      }
      ...
      if (user.profile_image) {
        ...
        profileImageElement.src = 'https://i.pinimg.com/736x/5a/bd/98/5abd985735a8fd4adcb0e795de6a1005.jpg';
      }
      ...
      // Account info
      document.getElementById('newsletter-status').textContent = user.subscribe ? 'Subscribed' : 'Not Subscribed';
      document.getElementById('date-joined').textContent = new Date(user.created_at).toLocaleString();
      document.getElementById('last-login').textContent = stats.lastLogin ? new Date(stats.lastLogin).toLocaleString() : 'Never';

      // Security
      document.getElementById('two-fa-status').textContent = security.two_factor_enabled ? 'Enabled' : 'Disabled';
      document.getElementById('active-sessions').textContent = stats.currentSessions || 0;
      ...
      document.getElementById('active-tickets').textContent = supportTickets.activeCount;
      document.getElementById('total-tickets').textContent = supportTickets.totalCount;
      ...
      // Engagement
      document.getElementById('notification-stats').textContent = 
        `${notifications.total_notifications} total, ${notifications.unread_notifications} unread`;
      ...
      activity.slice(0, 10).forEach(act => {
        const li = document.createElement('li');
        li.innerHTML = `<strong>${act.activity_type}:</strong> ${act.description} <small>(${new Date(act.created_at).toLocaleString()})</small>`;
    ...
    async function changeUserStatus(action) {
      ...
      try {
        const response = await fetch(`${apiBase}/users/${currentUser.user.id}/status`, {
          method: 'POST',
          headers: {
    ...
    async function resetPassword() {
      ...
      try {
        const response = await fetch(`${apiBase}/users/${currentUser.user.id}/reset-password`, {
          method: 'POST',
          credentials: 'include',
          headers: {
            'x-csrf-token': window.getCsrfToken()
    ...
    async function sendEmailToUser() {
      ...
      try {
        const response = await fetch(`${apiBase}/users/${currentUser.user.id}/send-email`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-csrf-token': window.getCsrfToken()
